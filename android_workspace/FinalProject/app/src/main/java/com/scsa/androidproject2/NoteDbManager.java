...
public class NoteDbManager {
    ...
    private static final String[] DATABASE_COLUMNS = { "_id", "title", "body", "status", "date" };
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            sql.append(DATABASE_COLUMNS[3] + " INTEGER DEFAULT 0, \n"); // 상태 플래그 컬럼 추가
            sql.append(DATABASE_COLUMNS[4] + " text);"); // 날짜 컬럼 추가
            sql.append(");");
    ...
    public long insert(Note note) {
        ...
        values.put(DATABASE_COLUMNS[3], note.getStatus()); // 상태 플래그 추가
        values.put(DATABASE_COLUMNS[4], note.getDate());
    ...
    public int update(Note note) {
        ...
        values.put(DATABASE_COLUMNS[1], note.getTitle());
        values.put(DATABASE_COLUMNS[2], note.getBody());
        values.put(DATABASE_COLUMNS[3], note.getStatus()); // 상태 플래그 업데이트
        values.put(DATABASE_COLUMNS[4], note.getDate());
